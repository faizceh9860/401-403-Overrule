# -*- coding: utf-8 -*-
"""
401/403 Overrule Burp Extension
=========================
A Burp Suite extension (Jython) that reimplements the bypass techniques
and payload wordlists of the open-source tool "nomore403".
"""
...
# NOTE: Abbreviated to prevent script truncation. Paste your original full lists here if needed.
# ---------------------------------------------------------------------------
...
class BurpExtender(IBurpExtender, ITab, IContextMenuFactory, IExtensionStateListener):
    ...
    def _build_ui(self):
        ...
        # New Global "Run All Tabs" Button
        btn_run_all_tabs = JButton("Run All Tabs", actionPerformed=lambda e: self._on_tab_run("All"))
        style_button(btn_run_all_tabs, BTN_GREEN)
        settings_row.add(btn_run_all_tabs)
        ...
        def make_tab(name):
            ...
            run_btn = JButton("Run", actionPerformed=lambda e, tn=name: self._on_tab_run(tn))
            style_button(run_btn, BTN_GREEN)
            ...
            progress.setPreferredSize(Dimension(120, 18))
            progress.setStringPainted(True)
            status_label = JLabel("Ready.")
            
            for comp in (run_btn, stop_btn, resume_btn, sendsel_btn, sendall_btn, clearsel_btn, clearall_btn, progress, status_label):
                ctrl.add(comp)
            ...
            self._tab_data[name] = {
                ...
                "run_btn": run_btn, "stop_btn": stop_btn, "resume_btn": resume_btn,
                "sendsel_btn": sendsel_btn, "sendall_btn": sendall_btn,
                "progress": progress, "status_label": status_label,
            }
    ...
    def _send_to_tab(self, invocation):
        ...
        self._set_status("Loaded target: %s" % str(url))
    ...
    def _on_tab_run(self, tab_name):
        ...
        if self._active_tab is not None:
            self._busy_dialog()
            return
        if tab_name in ("All", "Baseline"):
            self._baseline_cache = None
        if tab_name == "All":
            self._clear_all_tabs()
        else:
            self._clear_tab_own(tab_name)
            self._remove_from_all_matching(tab_name)
    ...
    def _on_tab_stop(self, tab_name):
        ...
        self._set_tab_status(tab_name, "Stopping...")
    ...
    def _on_tab_clear_all(self, tab_name):
        if tab_name == "All":
            ...
            if tab_name == "Baseline":
                self._baseline_cache = None
        self._set_status("Cleared %s." % tab_name)
    ...
    def _on_tab_clear_selected(self, tab_name):
        ...
        if tab_name == "All":
            by_tab = {}
            for res in removed:
                tn = TECHNIQUE_KEY_TO_TAB.get(res.technique)
                if tn:
                    by_tab.setdefault(tn, []).append(res)
            for tn, lst in by_tab.items():
                self._remove_results_from_tab(tn, lst)
        else:
            self._remove_results_from_tab("All", removed)
    ...
    def _remove_from_all_matching(self, tab_name):
        keys = self._keys_for_tab(tab_name)
        matching = [r for r in self._tab_data["All"]["results"] if r.technique in keys]
        self._remove_results_from_tab("All", matching)
    ...
    def _set_tabs_busy(self, busy, active_tab):
        for name, data in self._tab_data.items():
            if data.get("run_btn") is None: continue
            if busy:
                data["run_btn"].setEnabled(False)
                data["sendall_btn"].setEnabled(False)
                data["sendsel_btn"].setEnabled(False)
                data["resume_btn"].setEnabled(False)
                data["stop_btn"].setEnabled(name == active_tab)
            else:
                data["run_btn"].setEnabled(True)
    ...
    def _set_tab_progress(self, tab_name, pct, text):
        data = self._tab_data.get(tab_name)
        if data and data.get("progress"):
            data["progress"].setValue(pct)
        self._set_tab_status(tab_name, text)

    def _set_tab_status(self, tab_name, text):
        data = self._tab_data.get(tab_name)
        if data and data.get("status_label"):
            data["status_label"].setText(text)
    ...
    def _send_op(self, op):
        ...
        req_bytes = None
        try:
            raw_resp = raw_http_request(host, port, use_https, op["method"], op["target"], op["headers"], op["body"], http_version=op["http_ver"], timeout_ms=10000)
            status, length = raw_resp.status, raw_resp.length
            location, content_type, server, body_hash = raw_resp.location, raw_resp.content_type, raw_resp.server, raw_resp.body_hash
            resp_bytes = raw_resp.raw_bytes
            req_preview = ("%s %s %s\r\n" % (op["method"], op["target"], op["http_ver"]) + "\r\n".join("%s: %s" % (n, v) for n, v in op["headers"]) + "\r\n\r\n" + (op["body"] or ""))
            req_bytes = bytearray(req_preview, "iso-8859-1")
        except Exception as e:
            status, length, resp_bytes = None, None, None
            location, content_type, server, body_hash = None, None, None, ""
    ...
    def _run_ops(self, tab_name, ops):
        ...
        try:
            try: baseline, tolerance = self._ensure_baseline()
                ...
                SwingUtilities.invokeLater(lambda tn=tab_name, msg="Baseline failed: %s" % str(e): self._set_tab_status(tn, msg))
                self._tab_data[tab_name]["pending_ops"] = list(ops)
                return

            total = len(ops)
            if total == 0:
                SwingUtilities.invokeLater(lambda tn=tab_name, m="Baseline refreshed." if tab_name == "Baseline" else "Nothing to send.": self._set_tab_status(tn, m))
            ...
            for i, op in enumerate(ops):
                ...
                SwingUtilities.invokeLater(lambda p=int(100.0 * done / total), d=done, t=total, tn=tab_name: self._set_tab_progress(tn, p, "Sending %d / %d..." % (d, t)))

            self._tab_data[tab_name]["pending_ops"] = remaining
        finally:
            self._active_tab = None
            SwingUtilities.invokeLater(lambda tn=tab_name: [self._set_tabs_busy(False, None), self._set_tab_status(tn, "Stopped -- %d pending." % len(self._tab_data[tn].get("pending_ops") or []) if len(self._tab_data[tn].get("pending_ops") or []) else "Done.")])
